// inspired from: https://github.com/ckeditor/ckeditor5/blob/master/packages/ckeditor5-image/src/imageutils.ts
 ...
 */
export default class MediaUtils extends Plugin {
  ...
  public static get pluginName() {
    return "MediaUtils" as const;
  }

  public isImage(
    modelElement?: Element | null
  ): modelElement is Element & { name: "imageInline" | "imageBlock" } {
  ...
  public isBlockImageView(element?: ViewElement | null): boolean {
    return !!element && element.is("element", "figure") && element.hasClass("media");
  }
  ...
  public insertImage(
    ...
    const determinedImageType = determineImageTypeForInsertion(
      editor,
    ...
    return model.change((writer) => {
      ...
      model.insertObject(imageElement, selectable, null, {
        ...
        findOptimalPosition:
          !selectable && determinedImageType != "imageInline" ? "auto" : undefined,
      });
   ...
   */
  public getClosestSelectedImageWidget(
  ...
  public isImageAllowed(): boolean {
    ...
    return isImageAllowedInParent(editor, selection) && isNotInsideImage(selection);
  }
  ...
  public findViewImgElement(figureView: ViewElement): ViewElement | undefined {
    ...
    for (const { item } of editingView.createRangeIn(figureView)) {
      if (this.isInlineImageView(item as ViewElement)) return item as ViewElement;
    }
  }
...
function isImageAllowedInParent(editor: Editor, selection: Selection | DocumentSelection): boolean {
  ...
  if (imageType == "imageBlock") {
    ...
    if (model.schema.checkChild(parent as Element, "imageBlock")) return true;
  } else if (model.schema.checkChild(selection.focus!, "imageInline")) {
    return true;
  }
...
function isNotInsideImage(selection: DocumentSelection): boolean {
  return [...selection.focus!.getAncestors()].every(
    (ancestor) => !ancestor.is("element", "imageBlock")
  );
}
...
function determineImageTypeForInsertion(
  ...
  const { model, plugins, config } = editor;
  const schema = model.schema;
  const configImageInsertType = config.get("image.insert.type");

  if (!plugins.has("ImageBlockEditing")) return "imageInline";
  if (!plugins.has("ImageInlineEditing")) return "imageBlock";
...
/** function determineImageTypeForInsertionAtSelection inspired from: https://github.com/ckeditor/ckeditor5/blob/master/packages/ckeditor5-image/src/image/utils.ts */
