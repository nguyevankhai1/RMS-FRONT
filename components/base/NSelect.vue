<template>
  <div>
    ...
    <VueMultiselect
      ...
      :placeholder="props.label ? $t(props.label): ''"
    ...
    >
      ...
      <template #noOptions>
        <div class="fs-6">
          {{ props.options.taggable ? $t("inputThenEnterToAddData") : $t("noData") }}
        </div>
      </template>
      <template #noResult>
        <div class="fs-6">{{ $t("noSuitableSearchResult") }}</div>
      </template>
      <template #option="{ option }" v-if="props.options.description">
        <div class="option__desc">
          <span class="option__title">
            {{ props.options.label == "name" && !option[props.options.label] ? option.account : option[props.options.label]}}
            <span v-if="props.options.description == 'email'">
              (
              {{ !option[props.options.description] ? option.phone : option[props.options.description]}}
              )
            </span>
            <span v-else>
              (
              {{ $t(option[props.options.description])}}
              )
            </span>
          </span>
        </div>
  ...
  get:() => {
    ...
    if (props.returnObject || !value) {
      ...
      if (props.options.multiple && !props.options.taggable) {
        ...
        return (props.modelValue as string).split(CHARACTER_JOIN).map((element: any) => {
          return { label: element };
        });
      } else {
        if (props.modelValue && (slotOptions.value || []).length != 0) {
          initValue.value = true
        }
        return slotOptions.value.find((element: any) => element[props.options.value] == props.modelValue);
      }
    }
  },
  set: (value: any) => {
...
const getValue = () => {
  ...
  for (let i = 0; i < output.length; i++) {
    output[i] = slotOptions.value.find((element: any) => element[props.options.value] == output[i]);
  }
...
const loadData = async (type?: string) => {
  ...
  const data = await service.getForSelect(deepCopy(params.value), props.options.path);
  ...
  if (props.options.isNotCurrentValue) {
    slotOptions.value = slotOptions.value.filter(
      (item: any) => item[props.options.value] != props.data[props.options.value]
    );
  }
...
const addSelfIfNeeded = async () => {
  const selfId = props.modelValue.toString();
  const listValue = selfId.split(",");
  listValue.map((e, idx)  => {
    const obj = slotOptions.value.find((i: any) => i[props.options.value] == e);
    if (obj) {
        selectedObject.value[idx] = obj;
    } else {
        service.finById(e).then((data: any) => {
          if (data.value.data) {
            slotOptions.value.push(data.value.data)
            selectedObject.value = selected.value
          }
        })
    }
  });
...
const handleDisabled = () => {
  if ((props.options.disabledItems || []).length !== 0) {
    ...
    slotOptions.value.map((e: any) => {
      if (
        props.options.disabledItems?.find((f: any) => f === e[props.options.value]) &&
...
watch(
  () => selectedObject.value,
  () => {
    selected.value = selectedObject.value;
  }
);
